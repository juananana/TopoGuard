...
| P-3 分离拓扑/资源贡献 | §4.5 新增回归分析段落：**88.7%** 的质量优势来自纯拓扑选择 |
...
## 八、新实验结果（2026-04-25，500 轮水文问答）

### 8.1 核心模块消融（Bootstrap 95% CI + Wilcoxon）

新增 `--ablation` 入口，在 reuse 模式下运行，输出至 `ablation_and_constraints.json`。

| 变体 | $S$ | ΔS | $p$ (Wilcoxon) |
|------|-----|-----|----------------|
| TopoGuard（完整） | 0.823 [0.803, 0.842] | — | — |
| **w/o Local Repair** | **0.796** | **$+0.027$** | **$<0.001$*** |
| **w/o Template Selection** | **0.781** | **$+0.042$** | **$<0.001$*** |
| **w/o Executor Adaptation** | **0.744** | **$+0.079$** | **$<0.001$*** |
| w/o Bayesian Calibration | 0.823 | $0.000$ | n.s. |

**关键发现：**
- Bootstrap 95% CI：$S=0.823$ [0.803, 0.842]
- 贡献排序：Executor Adaptation (+0.079) > Template Selection (+0.042) > Local Repair (+0.027)
- w/o BC = TopoGuard（PM 冷启动退化，fallback 到 inline Pareto+Q）
- **修复设计说明**：
  1. `actual_for_topo` 改为 `actual_for_model_topo`，用 selected_model 查质量（而非 test_model）
  2. w/o Template Selection 改为随机 topo 选择（而非强制最深 topo）
  3. ProfileManager fallback 条件：PM 候选数 ≤ 1 时 fallback 到 inline Pareto+Q
...
| 修复触发率 | 87.0% (587/675) |
| 均值增益 | $+0.117$ / 修复 |
| w/o Repair 基线 | $S=0.701$（正确值） |
| 策略 C1（跨拓扑升级） | 438 例（74.6%） |
| 策略 A（拓扑升级） | 87 例（14.8%） |
| 策略 B（执行器升级） | 58 例（9.9%） |

### 8.3 约束鲁棒性

$C_\max \in \{0.3,0.4,0.5,0.6,0.7\}$ 五个水平均达到 100% 可行上下文覆盖率，说明当前实验设置的成本约束未成为拓扑选择瓶颈，质量阈值 $\tau_\mathrm{pass}$ 是更关键的绑定约束。

### 8.4 Wilcoxon 显著性（全基线）

TopoGuard vs 所有基线均为 $p<0.001$***（255 配对上下文），除 vs FrugalGPT Cascade（$p=0.017^*$）外均达到极高显著性。

### 8.5 F-3 边际效益分解（更新值）

| 指标 | 值 |
|------|-----|
| 总 ΔS（vs Static Workflow） | +0.063 |
| 纯拓扑增益 β₀ | +0.0561 |
| 边际回报 β₁ | +0.018 |
| **拓扑贡献比** | **88.7%** |
| Cohen's d | 0.46 |
